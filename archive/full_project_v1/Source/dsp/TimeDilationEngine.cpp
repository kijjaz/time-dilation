...
void TimeDilationEngine::renderTrackSynth (int trackIndex, juce::AudioBuffer<float>& trackBuffer, int numSamples)
{
    ...
    const float midiNote = track.steps[currentStep] ? static_cast<float>(track.stepNotes[currentStep]) : (52.0f + static_cast<float>(trackIndex * 5));
    ...
    const float phaseInc = static_cast<float>(2.0 * juce::MathConstants<double>::pi * noteFreq / sampleRate);
    const float gain = track.steps[currentStep] ? 0.35f : 0.15f;

    for (int s = 0; s < numSamples; ++s)
    {
        float val = std::sin (phase) * gain;
        left[s] += val;
        if (right != nullptr) right[s] += val;
