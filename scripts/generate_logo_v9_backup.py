...
def render_velocity_dependent_trails(particles, bh_x, width):
    # Renders per-segment movement trails where LOW velocity = Brighter/Thicker, HIGH velocity = Faint/Tapered!
    svg_parts = []
    svg_parts.append('<!-- DYNAMIC VELOCITY TRAILS: Low velocity = Brighter/Thicker, High velocity = Faint/Motion-Blurred -->')
            ...
            if len(p.history) >= 2:
                ...
                for k in range(len(p.history) - 1):
                    ...
                    if -60 <= x1 <= width + 60 and -60 <= x2 <= width + 60:
                        ...
                        # RELATIVISTIC VELOCITY TRAIL SHADING MATH:
                        # Low velocity (v < 4.0): Brighter & Thicker! (opacity ~ 0.85, width x1.2)
                        # High velocity (v > 8.0): Faint & Motion-Blurred! (opacity ~ 0.22, width x0.6)
                        opacity = max(0.18, min(0.88, 0.90 - (v_avg / 12.0) * 0.68))
                        stroke_w = max(0.6, base_w * (1.3 - (v_avg / 12.0) * 0.65))

                        # Color transition along trajectory
                        head_c, trail_c = get_particle_color(x1, p.color_seed, bh_x)

                        # If velocity is low, shift color towards brighter head color!
                        if v_avg < 4.5:
                            seg_col = head_c
                        else:
                            seg_col = trail_c
...
def create_time_dilation_logo_v9():
    width = 1000
    height = 600
    ...
    # 4. VELOCITY-DEPENDENT MOVEMENT TRAILS (Low velocity = Brighter/Thicker, High velocity = Faint/Blurred!)
    svg_parts.append(render_velocity_dependent_trails(particles, bh_x, width))
...
def create_icon_only_logo_v9():
    ...
    <!-- Velocity-Dependent Trails in Icon -->
    <g filter="url(#subtleGlowIcon)">
    ''')
...
if __name__ == "__main__":
    ...
    banner_svg = create_time_dilation_logo_v9()
    icon_svg = create_icon_only_logo_v9()
    ...
    print("v9 Relativistic Velocity-Shaded Movement Trails Logo generated successfully in Source/assets/ and web/")
