...
class Particle:
    def __init__(self, x, y, vx, vy, radius, particle_type="standard", mass=1.0):
        ...
        self.mass = mass
        self.history = [(x, y)]
        self.color_seed = random.uniform(0, 1)
...
def run_pure_particle_simulation(num_steps=160, width=1000, height=600):
    ...
    for step in range(num_steps):
        for p in particles:
            ...
            if v_mag > 12.0:
                p.vx = (p.vx / v_mag) * 12.0
                p.vy = (p.vy / v_mag) * 12.0
...
def get_particle_color(x, color_seed, bh_x):
    if x < bh_x - 80:
        if color_seed > 0.6:
            head = "#0284c7"
            trail = "#07364b"
...
def render_td_monogram(cx, cy, scale=1.0):
    # Futuristic Relativistic "TD" Monogram
    # Merged T & D letters with relativistic warp curves and dark drop shadow
    stroke_w = 4.5 * scale
    out = []
    ...
    <g opacity="0.98" filter="url(#subtleGlow)">
        ...
        <!-- T Top Bar in Relativistic Gold -->
        ...
        <!-- T Stem & D Vertical Axis in Copper Orange -->
        ...
        <!-- D Outer Curve in Deep Magenta -> Gold Gradient Sweep -->
        ...
        <!-- Inner Relativistic Gamma Dot / Quantum Focus Point -->
...
def create_time_dilation_logo_v8():
    width = 1000
    height = 600
    ...
    <!-- 1. Gravitational Light Lensing Arc to Camera -->
    ...
    # 4. DARK MOVEMENT TRAILS (Darker than particles)
    svg_parts.append('<!-- DARK KINETIC MOVEMENT TRAILS -->')
    svg_parts.append('<g filter="url(#subtleGlow)">')

    for p in particles:
        if p.particle_type in ("macro", "medium") or (p.particle_type == "standard" and random.random() > 0.4):
            if len(p.history) > 2:
                pts_str = " ".join([f"{hx:.1f},{hy:.1f}" for hx, hy in p.history if -60 <= hx <= width+60])
                if pts_str:
                    _, trail_col = get_particle_color(p.x, p.color_seed, bh_x)
                    stroke_w = max(0.9, p.radius * 0.55)
                    svg_parts.append(f'<polyline points="{pts_str}" fill="none" stroke="{trail_col}" stroke-width="{stroke_w:.1f}" opacity="0.80" stroke-linecap="round"/>')

    svg_parts.append('</g>')

    # 5. PURE PARTICLES (Ranging from micro specs up to heavy macro time nodes!)
    svg_parts.append('<!-- Pure Physics Particles -->')
    ...
    for p_type in ("micro", "standard", "medium", "macro"):
        ...
        for p in particles:
            if p.particle_type == p_type and -30 <= p.x <= width + 40 and -30 <= p.y <= height + 40:
                head_col, _ = get_particle_color(p.x, p.color_seed, bh_x)
    ...
    # 6. Black Hole Event Horizon & RELATIVISTIC "TD" MONOGRAM LOGO SYMBOL
...
def create_icon_only_logo_v8():
    ...
    <!-- Dark Movement Trails in Icon -->
    <g filter="url(#subtleGlowIcon)">
    ''')

    for p in particles[::4]:
        if len(p.history) > 2:
            pts_str = " ".join([f"{hx*0.5+40:.1f},{hy*0.8+50:.1f}" for hx, hy in p.history if -30 <= hx <= width+30])
            if pts_str:
                _, trail_col = get_particle_color(p.x, p.color_seed, bh_x)
                svg_parts.append(f'<polyline points="{pts_str}" fill="none" stroke="{trail_col}" stroke-width="1.2" opacity="0.80" stroke-linecap="round"/>')
...
if __name__ == "__main__":
    ...
    banner_svg = create_time_dilation_logo_v8()
    icon_svg = create_icon_only_logo_v8()
    ...
    print("v8 Relativistic TD Monogram Logo in Black Hole Center generated successfully in Source/assets/ and web/")
